...
void calculateDCT_OMPD(array<float, 64> &block, array<double, 64> &output, int threads) {
...
void dct_openmp_dynamic::run(image & img, array<vector<array<float, 64>>, 3>& blocks, array<vector<array<double, 64>>, 3>& DCT, int *args) {
	...
	for (rows = 0; rows < img.BlockCountY(); rows++) {
		for (int columns = 0; columns < img.BlockCountX(); columns++) {
			...
			for (components = 0; components < 3; components++) {
				calculateDCT_OMPD(blocks[components][index], DCT[components][index], threadCount);
			}
		}
	}
}